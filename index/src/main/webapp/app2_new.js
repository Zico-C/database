...
function addToCart() {
  ...
  const total = Array.from(totalPrices).reduce((acc, el) => acc + parseInt(el.innerText), 0);
  document.getElementById('totals').innerText = total; // 更新總價格
...
function initializeCart() {
  ...
  for (const cartItem of existingCartItems) {
    ...
    cartItemElement.innerHTML = `
        ...
        <button class="removeButton1" id="delete">删除</button>
        </div>
    ...
    removeButton1.addEventListener('click', removeCartItem1);
...
document.getElementById('totals').innerText = total; // 更新總價格
  ...
  const total = totalPriceArray.reduce((acc, price) => acc + parseInt(price), 0);
  document.getElementById('totals').innerText = total; // 更新總價格
});
